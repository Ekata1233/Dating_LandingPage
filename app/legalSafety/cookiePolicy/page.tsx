...
export default function CookiePolicyPage() {
  const { cookiePolicy, loading, error } = useLegalData();

  if (loading) return <LoadingSkeleton />;
  if (error || !cookiePolicy) return <ErrorState message={error || "Cookie policy not available."} />;

  const { content, effectiveFrom, publishedAt } = cookiePolicy;
