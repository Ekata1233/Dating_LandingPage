...
/*  Page                                                               */
/* ------------------------------------------------------------------ */
export default function AgePolicy18PlusPage() {
  const { agePolicy18Plus, loading, error } = useLegalData();

  if (loading) return <LoadingSkeleton />;
  if (error || !agePolicy18Plus) return <ErrorState message={error || "Age policy for 18+ not available."} />;

  const { content, effectiveFrom, publishedAt } = agePolicy18Plus;
