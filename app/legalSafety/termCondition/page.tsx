...
export default function TermsOfServicePage() {
  const { termsOfService, loading, error } = useLegalData();

  if (loading) return <LoadingSkeleton />;
  if (error || !termsOfService) return <ErrorState message={error || "Terms of service not available."} />;

  const { content, effectiveFrom, publishedAt } = termsOfService;
