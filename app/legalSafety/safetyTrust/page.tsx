...
export default function SafetyAndTrustPage() {
  const { safetyAndTrust, loading, error } = useLegalData();

  if (loading) return <LoadingSkeleton />;
  if (error || !safetyAndTrust) return <ErrorState message={error || "Safety and trust information not available."} />;

  const { content, effectiveFrom, publishedAt } = safetyAndTrust;
