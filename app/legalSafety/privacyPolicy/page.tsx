...
export default function PrivacyPolicyPage() {
  const { privacyPolicy, loading, error } = useLegalData();

  if (loading) return <LoadingSkeleton />;
  if (error || !privacyPolicy) return <ErrorState message={error || "Privacy policy not available."} />;

  const { content, effectiveFrom, publishedAt } = privacyPolicy;
