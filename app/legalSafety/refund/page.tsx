...
export default function RefundAndCancellationPage() {
  const { refundAndCancellation, loading, error } = useLegalData();

  if (loading) return <LoadingSkeleton />;
  if (error || !refundAndCancellation) return <ErrorState message={error || "Refund and cancellation policy not available."} />;

  const { content, effectiveFrom, publishedAt } = refundAndCancellation;
